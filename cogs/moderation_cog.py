...
class ModerationCog(commands.Cog):
    ...
    async def _fake_moderation_response(self, action, target, reason=None, duration=None):
        ...
        responses = {
            "ban": [
                f"🔨 **Banned {target}**! Reason: {reason or 'No reason provided'}",
                f"👋 {target} has been banned from the server. Reason: {reason or 'No reason provided'}",
                f"🚫 {target} is now banned. Reason: {reason or 'No reason provided'}"
            ],
            "kick": [
            ...
            "mute": [
                f"🔇 **Muted {target}**! Reason: {reason or 'No reason provided'}",
                f"🤐 {target} has been muted. Reason: {reason or 'No reason provided'}",
                f"📵 {target} can no longer speak. Reason: {reason or 'No reason provided'}"
            ],
            "timeout": [
    ...
    async def ban(self, ctx: commands.Context, member: discord.Member = None, *, reason: str = None):
        ...
        response = await self._fake_moderation_response("ban", member.mention, reason)
    ...
    async def mute(self, ctx: commands.Context, member: discord.Member = None, *, reason: str = None):
        """Pretends to mute a member in the server."""
        if not member:
            await ctx.reply("Please specify a member to mute.")
            return
            
        response = await self._fake_moderation_response("mute", member.mention, reason)
        await ctx.reply(response)
    ...
    async def mute_slash(self, interaction: discord.Interaction, member: discord.Member, reason: str = None):
