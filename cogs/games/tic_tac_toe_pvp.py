...
# --- Tic Tac Toe (Player vs Player) --- START
...
async def tictactoe_slash(interaction: discord.Interaction, opponent: discord.Member):
...
async def tictactoe_prefix(ctx: commands.Context, opponent: discord.Member):
...
async def setup(bot: commands.Bot, cog: commands.Cog):
    tree = bot.tree
    tree.add_command(tictactoe_slash, guild=cog.guild)
    bot.add_command(tictactoe_prefix)
