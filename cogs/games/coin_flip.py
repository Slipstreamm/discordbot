...
# --- Coin Flip Game (Player vs Player) ---
...
async def coinflipbet_slash(interaction: discord.Interaction, opponent: discord.Member):
...
async def coinflipbet_prefix(ctx: commands.Context, opponent: discord.Member):
...
# --- Setup Function ---
# This function will be called by the main cog to add the commands

async def setup(bot: commands.Bot, cog: commands.Cog):
    # Add slash commands to the tree
    tree = bot.tree
    tree.add_command(coinflipbet_slash, guild=cog.guild) # Assuming commands are guild-specific
    tree.add_command(coinflip_slash, guild=cog.guild)

    # Add prefix commands to the bot
    bot.add_command(coinflipbet_prefix)
    bot.add_command(coinflip_prefix)
