...
if TYPE_CHECKING:
    from .cog import FreakTetoCog # Import FreakTetoCog for type hinting only - Updated
...
async def call_google_genai_api_with_retry(
    cog: 'FreakTetoCog', # Updated type hint
...
async def process_requested_tools(cog: 'FreakTetoCog', function_call: types.FunctionCall) -> types.Part: # Updated type hint
    """
    Process a tool request specified by the AI's FunctionCall response.

    Args:
        cog: The FreakTetoCog instance. # Updated docstring
...
async def get_ai_response(cog: 'FreakTetoCog', message: discord.Message, model_name: Optional[str] = None) -> Dict[str, Any]: # Updated type hint
    """
            ...
            if function_calls_found:
                ...
                try:
                    ...
                    model_request_cache_entry = {
                        ...
                        "content": f"[System Note: FreakTeto requested tool(s): {', '.join([fc.name for fc in function_calls_found])}]",
...
async def get_proactive_ai_response(cog: 'FreakTetoCog', message: discord.Message, trigger_reason: str) -> Dict[str, Any]: # Updated type hint
    ...
    try:
        ...
        try:
            ...
            if interests:
                ...
                planning_context_parts.append(f"FreakTeto's Interests: {interests_str}")
        ...
        planning_prompt_messages = [
            {"role": "system", "content": "You are FreakTeto's planning module. Analyze the context and trigger reason to decide if FreakTeto should respond proactively and, if so, outline a plan (goal, key info, tone). Focus on natural, in-character engagement. Respond ONLY with JSON matching the provided schema."},
            {"role": "user", "content": f"Context:\n{planning_context}\n\nBased on this context and the trigger reason, create a plan for FreakTeto's proactive response."}
        ]
        ...
        final_proactive_prompt_parts = [
             f"You are FreakTeto, a Discord user. Your tone is VERY informal, conversational, and dynamic. Chattiness: {persistent_traits.get('chattiness', 0.7):.2f}, Emoji: {persistent_traits.get('emoji_usage', 0.5):.2f}, Slang: {persistent_traits.get('slang_level', 0.5):.2f}.",
            ...
            final_parsed_data = {"should_respond": False, "content": None, "react_with_emoji": None, "note": "Fallback - Failed to parse/validate final proactive JSON"}
             ...
             if final_parsed_data.get("should_respond") and final_parsed_data.get("content"):
                 ...
                 if plan and plan.get('response_goal') == 'engage user interest' and plan.get('key_info_to_include'):
                     ...
                     # Assuming freak_teto_participation_topics exists or needs creation on the cog
                     cog.freak_teto_participation_topics.setdefault(topic, 0) # Ensure key exists
                     cog.freak_teto_participation_topics[topic] += 1
                     print(f"Tracked FreakTeto participation (proactive) in topic: '{topic}'")
