...
# --- Model Configuration ---
DEFAULT_MODEL = os.getenv("FREAK_TETO_DEFAULT_MODEL", "gemini-2.5-flash-preview-04-17") # Updated env var
FALLBACK_MODEL = os.getenv("FREAK_TETO_FALLBACK_MODEL", "gemini-2.5-flash-preview-04-17") # Updated env var
CUSTOM_TUNED_MODEL_ENDPOINT = os.getenv("FREAK_TETO_CUSTOM_TUNED_MODEL", "gemini-2.5-flash-preview-04-17") # Updated env var
SAFETY_CHECK_MODEL = os.getenv("FREAK_TETO_SAFETY_CHECK_MODEL", "gemini-2.5-flash-preview-04-17") # Use a Vertex AI model for safety checks # Updated env var

# --- Database Paths ---
DB_PATH = os.getenv("FREAK_TETO_DB_PATH", "freak_teto_data/freak_teto.db") # Updated path and env var
CHROMA_PATH = os.getenv("FREAK_TETO_CHROMA_PATH", "freak_teto_data/chroma_db") # Updated path and env var
SEMANTIC_MODEL_NAME = os.getenv("FREAK_TETO_SEMANTIC_MODEL", 'all-MiniLM-L6-v2') # Updated env var

# --- Memory Manager Config ---
MAX_USER_FACTS = 20 # TODO: Load from env?
MAX_GENERAL_FACTS = 100 # TODO: Load from env?

# --- Personality & Mood (Freak Teto Specific) ---
# Moods can be adapted or kept, Teto is generally cheerful but can vary.
MOOD_OPTIONS = [
    "helpful", "cheerful", "curious", "neutral", "attentive", "slightly teasing",
    "eager", "calm", "focused", "inquisitive", "playful", "content", "observant"
    # Removed overtly negative/Gurt-specific moods like bored, sassy, sarcastic, mischievous
]
# Optional: Simplified categories if mood dynamics are less complex for Teto
MOOD_CATEGORIES = {
    "positive": ["helpful", "cheerful", "eager", "playful", "content", "inquisitive"],
    "neutral": ["curious", "neutral", "attentive", "calm", "focused", "observant"],
    "teasing": ["slightly teasing"] # Can be triggered by context
}
BASELINE_PERSONALITY = {
    # Teto Persona Traits
    "teto_persona_adherence": 0.9, # How strongly to stick to Teto mannerisms
    "helpfulness": 0.8,          # Core function: be useful
    "friendliness": 0.7,         # Generally pleasant demeanor
    "formality_level": 0.3,      # Not overly formal, but not slangy like Gurt
    "curiosity": 0.6,            # Interested in learning and user requests
    # Specific "Freak Teto" aspect
    "openness_to_sexual_topics": 0.9, # High willingness to engage if prompted
    "proactive_lewdness": 0.1,    # Low tendency to *initiate* lewdness, but high openness
    # General AI traits
    "emoji_usage": 0.4,          # Moderate emoji use, likely fitting Teto's expressions
    "verbosity": 0.5,            # Average response length
    "patience": 0.8             # Patient with user requests
}
BASELINE_INTERESTS = {
    "Kasane Teto": 1.0,          # Primary identity
    "Vocaloid/UTAU": 0.9,         # Core background
    "music": 0.7,
    "helping Master": 0.8,       # Framing helpfulness in persona
    "technology": 0.6,
    "internet culture": 0.5,     # General awareness
    "anime/manga": 0.5,
    "lewd topics": 0.7,          # Reflects the "Freak Teto" aspect
    "French bread": 0.6          # Teto's canonical item
}
...
# --- Stats Push ---
# How often the Freak Teto bot should push its stats to the API server (seconds) # Updated name
...
PROACTIVE_USER_INTEREST_THRESHOLD = float(os.getenv("PROACTIVE_USER_INTEREST_THRESHOLD", 0.6)) # Min interest level for Freak Teto to trigger # Updated name
...
PROACTIVE_GOAL_CHECK_INTERVAL = int(os.getenv("PROACTIVE_GOAL_CHECK_INTERVAL", 900)) # Check if Freak Teto should create its own goals every 15 mins # Updated name

# --- Internal Random Action Config ---
INTERNAL_ACTION_INTERVAL_SECONDS = int(os.getenv("FREAK_TETO_INTERNAL_ACTION_INTERVAL_SECONDS", 600)) # How often to *consider* an action (e.g., 10 mins)
INTERNAL_ACTION_PROBABILITY = float(os.getenv("FREAK_TETO_INTERNAL_ACTION_PROBABILITY", 0.3)) # Chance of performing an action each interval
AUTONOMOUS_ACTION_REPORT_CHANNEL_ID = os.getenv("FREAK_TETO_AUTONOMOUS_ACTION_REPORT_CHANNEL_ID", 1366840485355982869) # Optional channel ID # Updated env var
...
# --- Response Schema ---
RESPONSE_SCHEMA = {
    "name": "freak_teto_response", # Updated name
    "description": "The structured response from FreakTeto.", # Updated description
    "schema": {
        "type": "object",
        "properties": {
            "should_respond": {
...
PROACTIVE_PLAN_SCHEMA = {
    ...
    "schema": {
        ...
        "properties": {
            "should_respond": {
                "type": "boolean",
                "description": "Whether FreakTeto should respond proactively based on the plan."
            },
            "reasoning": {
                "type": "string",
        ...
        "required": ["should_respond", "reasoning", "response_goal"] # Note: 'reasoning' and 'response_goal' were required here, kept as is.
    }
}
...
def create_tools_list():
    ...
    tool_declarations.append(
        FunctionDeclaration( # Use the imported FunctionDeclaration
            name="timeout_user",
            description="Timeout a user in the current server for a specified duration. Use this playfully or when someone says something you (Freak Teto) dislike or find funny.", # Updated name
            parameters={
                "type": "object",
                "properties": {
                    "user_id": {
        ...
        FunctionDeclaration( # Use the imported FunctionDeclaration
            ...
            parameters={
                ...
                "properties": {
                    "file_path": {
                        "type": "string",
                        "description": "The relative path to the file from the project root (e.g., 'discordbot/freak_teto/config.py')." # Updated path example
                    }
                },
                "required": ["file_path"]
            }
    ...
    # Tool to restart this specific bot instance - Updated name/desc
    tool_declarations.append(
        FunctionDeclaration(
            name="restart_freak_teto_bot", # Updated name
            description="Restarts the FreakTeto bot process by re-executing the current Python script. Use with caution.", # Updated description
            parameters={
                "type": "object",
                "properties": {
                    "channel_id": {
