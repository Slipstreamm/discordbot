...
# Relative imports from within the freak_teto package and parent # Changed package name
...
async def timeout_user(cog: commands.Cog, user_id: str, duration_minutes: int, reason: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if member == cog.bot.user: return {"error": "I can't timeout myself, Master."}
        ...
        timeout_reason = reason or "Freak Teto felt like it" # Changed reason
...
async def remove_timeout(cog: commands.Cog, user_id: str, reason: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        timeout_reason = reason or "Freak Teto decided to be nice." # Changed reason
...
async def execute_internal_command(cog: commands.Cog, command: str, timeout_seconds: int = 60, user_id: str = None) -> Dict[str, Any]:
    ...
    WARNING: This tool is intended ONLY for internal bot operations and MUST NOT # Made generic
...
async def restart_freak_teto_bot(cog: commands.Cog, channel_id: str = None) -> Dict[str, Any]: # Renamed function
    """
    Restarts the Freak Teto bot process by re-executing the current Python script.
    ...
    try:
        await send_discord_message(cog, channel_id, "Restart tool was called.") # Kept message generic for now
    except Exception as msg_exc:
        print(f"Failed to send restart message: {msg_exc}")
    try:
        python = sys.executable
        os.execv(python, [python] + sys.argv)
        return {"status": "restarting", "message": "Freak Teto bot is restarting..."} # Changed message
    except Exception as e:
...
async def create_new_tool(cog: commands.Cog, tool_name: str, description: str, parameters_json: str, returns_description: str) -> Dict[str, Any]:
    ...
    system_prompt = (
        "You are a Python code generation assistant for Freak Teto, a Discord bot. " # Changed bot name
    ...
    user_prompt = (
        f"Create a new tool named '{tool_name}' for the Freak Teto bot.\n" # Added context
    ...
    tools_py_path = "discordbot/freak_teto/tools.py" # Changed path
    try:
    ...
    config_py_path = "discordbot/freak_teto/config.py" # Changed path
    try:
...
async def move_user_to_voice_channel(cog: commands.Cog, user_id: str, target_channel_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        await member.move_to(target_channel, reason="Moved by Freak Teto tool") # Changed reason
...
async def assign_role_to_user(cog: commands.Cog, user_id: str, role_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        await member.add_roles(role, reason="Assigned by Freak Teto tool") # Changed reason
...
async def remove_role_from_user(cog: commands.Cog, user_id: str, role_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        await member.remove_roles(role, reason="Removed by Freak Teto tool") # Changed reason
...
async def purge_messages(cog: commands.Cog, limit: int, channel_id: Optional[str] = None, user_id: Optional[str] = None, before_message_id: Optional[str] = None, after_message_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        deleted_messages = await channel.purge(
            ...
            before=before_obj,
            after=after_obj,
            reason="Purged by Freak Teto tool" # Changed reason
        )
...
TOOL_MAPPING = {
    ...
    "restart_freak_teto_bot": restart_freak_teto_bot, # Renamed tool
